...
const int max_amplitude = 32760; //16 bit number
...
int main(){
    ...
    if(wav.is_open()){
        ...
        for(int i = 0; i < sample_rate * duration; i++){
            //Make sure max amplitude doesnt go too high and make it act like a wave
            double amplitude = (double)i / sample_rate * max_amplitude;
            double value = sin((2 * 3.14 * i * frequency) / sample_rate);

            double channel1 = amplitude * value / 2; //low to high
            double channel2 = (max_amplitude - amplitude) * value; // high to low
